...
describe Student do
  ...
  describe '.find_by_name' do
    it 'executes the SQL to find a row by name' do
      ...
      expect(Student.find_by_name("Jan")).to eq([{"id"=>2, "name"=>"Jan", "grade"=>10}])
    end
  end
  ...
  describe '.find_by' do
    it 'executes the SQL to find a row by the attribute passed into the method' do
      ...
      expect(Student.find_by({name: "Susan"})).to eq([{"id"=>3, "name"=>"Susan", "grade"=>10}])
    end

    it 'accounts for when an attribute value is an integer' do
      Student.new({name: "Susan", grade: 10}).save
      Student.new({name: "Geraldine", grade: 9}).save
      expect(Student.find_by({grade: 10})).to eq([{"id"=>2, "name"=>"Jan", "grade"=>10}, {"grade"=>10, "id"=>3, "name"=>"Susan"},
{"grade"=>10, "id"=>4, "name"=>"Susan"}])
    end
  end
